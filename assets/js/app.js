...
class App {
  ...
  #createLayoutResizeControls() {
    ...
    if (null !== sidebarResizerHandler) {
      sidebarResizerHandler.addEventListener("click", () => {
        const oldValue =
          localStorage.getItem(this.#sidebarWidthLocalStorageKey) || "normal";
    ...
    if (null !== contentResizerHandler) {
      contentResizerHandler.addEventListener("click", () => {
        const oldValue =
          localStorage.getItem(this.#contentWidthLocalStorageKey) || "normal";
  ...
  #createModalWindowsForDeleteActions() {
    document
      .querySelectorAll('[data-action-name="delete"]')
  ...
  #createModalWindowsForConfirmationActions() {
    ...
    modalTitles.forEach((modalTitle) => {
      ...
      modalTitle.textContent = titleContentWithPlaceholders.replace(
        "%action_name%",
        actionName
      );
    });
      ...
      .forEach((comparisonWidget) => {
        comparisonWidget.addEventListener("change", (event) => {
          ...
          toggleVisibilityClasses(
            secondValue,
            comparisonWidget.value !== "between"
          );
        });
      });
  }
