...
.. note::
    ...
    ``html5 = true`` options in the underlying ``DateTimeType`` Symfony field.
...
.. note::
    ...
    Setting this option is equivalent to setting ``widget = single_text`` and
    ``html5 = false`` options in the underlying ``DateTimeType`` Symfony field.
...
Use this option to override that default formatting::
    ...
    // these are the predefined formats: 'none', 'short', 'medium', 'long', 'full'
    // the second argument is the time format and it defaults to 'none'
