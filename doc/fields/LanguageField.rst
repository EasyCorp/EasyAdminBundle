...
See also :doc:`LocaleField </fields/LocaleField>` and
...
By default, the language selector displays all the languages defined by the
`ICU project`_. Use this option to only display the given language codes::
...
By default, the language selector displays all the languages defined by the
`ICU project`_. Use this option to remove the given language codes from that
list::
...
The code is displayed as a badge whose tooltip shows the full language name.
...
Use this option to hide the language name. Combine it with ``showCode()`` to
display the language code instead; otherwise the field displays nothing::
