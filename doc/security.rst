...
.. _security-dynamic-controllers:

Restrict CRUD Controllers Dynamically
-------------------------------------
...
method in your dashboard controllers and return ``false`` for all the CRUD controllers that
should not be linked to that dashboard:
...
.. code-block:: php
    ...
    class DashboardController extends AbstractDashboardController
    {
        ...
        {
            // only allow controllers in the same namespace as this dashboard
            return str_starts_with($controllerFqcn, __NAMESPACE__ . '\\');
        }
    }

In this example, the routes generated by EasyAdmin for all controllers in the ``App\Controller\Admin``
namespace (the same as the admin dashboard controller) will be available only for the admin
dashboard. Do the same for the customer dashboard, and you will have a clean separation of
