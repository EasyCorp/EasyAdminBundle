...
final class CollectionConfigurator implements FieldConfiguratorInterface
{
    ...
    public function configure(FieldDtoInterface $field, EntityDtoInterface $entityDto, AdminContext $context): void
    {
        ...
        $field->setFormTypeOptionIfNotSet(
            'allow_add',
            $field->getCustomOptions()->get(CollectionField::OPTION_ALLOW_ADD)
        );
        $field->setFormTypeOptionIfNotSet(
            'allow_delete',
            $field->getCustomOptions()->get(CollectionField::OPTION_ALLOW_DELETE)
        );
        ...
        $field->setFormTypeOptionIfNotSet(
            'entry_options.label',
            $field->getCustomOptions()->get(CollectionField::OPTION_SHOW_ENTRY_LABEL)
        );
        ...
        if (null === $field->getCustomOption(CollectionField::OPTION_ENTRY_IS_COMPLEX)) {
            ...
            $isSymfonyCoreFormType = null !== u($entryTypeFqcn ?? '')->indexOf(
        ...
        if (true === $field->getCustomOption(CollectionField::OPTION_ENTRY_USES_CRUD_FORM)) {
            ...
            if (null === $targetCrudControllerFqcn) {
                throw new \RuntimeException(
                    sprintf(
                        'The "%s" collection field of "%s" wants to render its entries using an EasyAdmin CRUD form. However, no CRUD form was found related to this field. You can either create a CRUD controller for the entity "%s" or pass the CRUD controller to use as the first argument of the "useEntryCrudForm()" method.',
                        $field->getProperty(),
                        $context->getCrud()?->getControllerFqcn(),
                        $targetEntityFqcn
                    )
            ...
            $crudEditPageName = $field->getCustomOption(
                CollectionField::OPTION_ENTRY_CRUD_EDIT_PAGE_NAME
            ...
            $crudNewPageName = $field->getCustomOption(
                CollectionField::OPTION_ENTRY_CRUD_NEW_PAGE_NAME
            ...
            try {
                ...
                throw new \RuntimeException(
                    sprintf(
                        'The "%s" collection field of "%s" uses the "useEntryCrudForm()" method, which requires Symfony 6.1 or newer to work. Upgrade your Symfony version or use instead the "setEntryType()" method to render the collection entries using a Symfony form.',
                        $field->getProperty(),
                        $context->getCrud()?->getControllerFqcn()
                    ), 0, $exception
                );
