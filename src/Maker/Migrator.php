...
final class Migrator
{
    ...
    private function addConfigureFiltersMethod(CodeBuilder $code, array $entityConfig): CodeBuilder
    {
        ...
        foreach ($entityConfig['list']['filters'] as $filterName => $filterConfig) {
            $filterFqcn = $this->guessFilterFqcnForProperty($filterConfig);
            $filterClassName = u($filterFqcn)->afterLast('\\')->toString();

            // needed to turn 'foo' into '$fooFilter' and 'foo.bar.baz' into '$fooBarBazFilter'
            $filterVariableName = u($filterName)->replace('.', ' ')->camel()->collapseWhitespace()->toString();

            $methodArguments = [$filterName];
            $filterLabel = $filterConfig['label'];
            $humanizedLabel = null === $filterLabel ? null : $this->humanizeString($filterLabel);
            // in EA2, an empty label means no label (same as FALSE in EA3)
            if ('' === $filterLabel) {
                $methodArguments[] = false;
            } elseif ($filterLabel !== $humanizedLabel) {
                // to keep config more concise, set the label explicitly only if
                // it's different from the autogenerated label
                $methodArguments[] = $filterLabel;
            }

            $code = $code
                ->_use($filterFqcn)
                ->_variableName($filterVariableName)->equals()->_staticCall($filterClassName, 'new', $methodArguments)->semiColon();
            $code = $code
                ->_variableName('filters')->_methodCallWithRawArguments('add', ['$'.$filterVariableName])->semiColon();
        }

        $code->newLine()->_return()->_variableName('filters')->semiColon();
        $code = $code->closeBrace();
    ...
    private function guessFilterFqcnForProperty(array $filterConfig): string
    {
        $filterType = $filterConfig['type'];

        $filterTypeToFqcn = [
            'EasyCorp\\Bundle\\EasyAdminBundle\\Form\\Filter\\Type\\ArrayFilterType' => ArrayFilter::class,
            'EasyCorp\\Bundle\\EasyAdminBundle\\Form\\Filter\\Type\\BooleanFilterType' => BooleanFilter::class,
            'EasyCorp\\Bundle\\EasyAdminBundle\\Form\\Filter\\Type\\ChoiceFilterType' => ChoiceFilter::class,
            'EasyCorp\\Bundle\\EasyAdminBundle\\Form\\Filter\\Type\\ComparisonFilterType' => ComparisonFilter::class,
            'EasyCorp\\Bundle\\EasyAdminBundle\\Form\\Filter\\Type\\DateTimeFilterType' => DateTimeFilter::class,
            'EasyCorp\\Bundle\\EasyAdminBundle\\Form\\Filter\\Type\\EntityFilterType' => EntityFilter::class,
            'EasyCorp\\Bundle\\EasyAdminBundle\\Form\\Filter\\Type\\NumericFilterType' => NumericFilter::class,
            'EasyCorp\\Bundle\\EasyAdminBundle\\Form\\Filter\\Type\\TextFilterType' => TextFilter::class,
        ];

        return $filterTypeToFqcn[$filterType] ?? TextFilter::class;
    }
