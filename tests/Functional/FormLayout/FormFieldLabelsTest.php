...
use EasyCorp\Bundle\EasyAdminBundle\Tests\Functional\Apps\DefaultApp\Controller\Synthetic\FormFieldLabelSyntheticCrudController;
use EasyCorp\Bundle\EasyAdminBundle\Tests\Functional\Apps\DefaultApp\Entity\Synthetic\FormTestEntity;

class FormFieldLabelsTest extends AbstractCrudTestCase
{
    ...
    {
        return FormFieldLabelSyntheticCrudController::class;
    }
    ...
    public function testFieldsLabelsInForms(): void
    {
        ...
        static::assertSelectorTextSame('label[for="FormTestEntity_id"]', 'ID', 'The "id" field does not define a label explicitly, so it is generated automatically based on the field name. Edge-case: fields named "id" are labeled as uppercase "ID" instead of the default title-case "Id" of the rest of the fields.');
        ...
        static::assertSelectorTextSame('label[for="FormTestEntity_name"]', 'Name', 'The "name" field uses NULL as the value of the label, which tells EasyAdmin to generate a label automatically as the title-case value of the field name.');
        ...
        static::assertSelectorNotExists('.form-group.field-description label', 'The "description" field uses FALSE as the value of the label, which means that no <label> element should be rendered for that field.');
        ...
        static::assertSelectorTextSame('label[for="FormTestEntity_email"]', '', 'The "email" field defines an empty string as the label, so it renders a <label> element to keep the design layout but without any contents inside.');
        ...
        static::assertSelectorTextSame('label[for="FormTestEntity_createdAt"]', 'Lorem Ipsum 1', 'The "createdAt" field defines a regular text string as its label (no HTML contents).');
        ...
        static::assertSame($crawler->filter('label[for="FormTestEntity_street"]')->html(), '<a href="https://example.com">Street</a> <b>Lorem</b> Ipsum', 'The "street" field defines its label as translatable string with HTML contents, which must be rendered instead of escaped (HTML is allowed in field labels).');
    }
    ...
    public function testFieldsLabelsOnDetailPage(): void
    {
        ...
        static::assertSelectorNotExists('.form-fieldset.fieldset-2 .form-fieldset-title-content', 'The "Fieldset 2" field defines its label as NULL, so its label element is not rendered.');
        static::assertStringContainsString('form-fieldset-no-header', $crawler->filter('.form-fieldset.fieldset-2')->attr('class'), 'Fieldsets without explicit labels get a special CSS class to adjust the page design.');
        static::assertSelectorTextSame('.field-group.field-name .field-label > div', 'Name', 'The "name" field uses NULL as the value of the label, which tells EasyAdmin to generate a label automatically as the title-case value of the field name.');
        ...
        static::assertSelectorNotExists('.form-fieldset.fieldset-3 .form-fieldset-title-content', 'The "Fieldset 3" field defines its label as FALSE, so its label element is not rendered.');
        static::assertStringContainsString('form-fieldset-no-header', $crawler->filter('.form-fieldset.fieldset-3')->attr('class'), 'Fieldsets without explicit labels get a special CSS class to adjust the page design.');
        static::assertSelectorNotExists('.field-group.field-description .field-label', 'The "description" field uses FALSE as the value of the label, which means that no <label> element should be rendered for that field.');
        ...
        static::assertSelectorNotExists('.form-fieldset.fieldset-4 .form-fieldset-title-content', 'The "Fieldset 4" field defines its label as an empty string, so its label element is not rendered.');
        static::assertStringContainsString('form-fieldset-no-header', $crawler->filter('.form-fieldset.fieldset-4')->attr('class'), 'Fieldsets without explicit labels get a special CSS class to adjust the page design.');
        static::assertSelectorTextSame('.field-group.field-email .field-label > div', '', 'The "email" field defines an empty string as the label, so it renders a <label> element to keep the design layout but without any contents inside.');
        ...
        static::assertSame('<a href="https://example.com">Fieldset</a> <b>8</b>', trim($crawler->filter('.form-fieldset.fieldset-8 .form-fieldset-title-content')->html()), 'The "Fieldset 8" defines its label as a text string with HTML contents, which must be rendered instead of escaped.');
        static::assertSelectorNotExists('.form-fieldset.fieldset-8.form-fieldset-no-header');
        static::assertSame(trim($crawler->filter('.field-group.field-street .field-label > div')->html()), '<a href="https://example.com">Street</a> <b>Lorem</b> Ipsum', 'The "street" field defines its label as translatable string with HTML contents, which must be rendered instead of escaped (HTML is allowed in field labels).');
    }
}
